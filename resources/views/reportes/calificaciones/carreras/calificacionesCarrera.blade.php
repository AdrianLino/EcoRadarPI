<x-app-layout>
    ...
    <div class="container">
        ...
        <form method="GET" action="{{ route('calificaciones.por_carrera', $carrera) }}" class="mb-4" id="cicloForm">
            <div class="form-row">
                <div class="col">
                    <label for="search">Buscar Alumno (por nombre o matrícula):</label>
                    <input type="text" name="search" id="search" class="form-control"
                        placeholder="Escribe el nombre o matrícula" value="{{ $search }}">
                </div>
                <div class="col">
                    ...
                    <select name="selected_ciclo" id="selected_ciclo" class="form-control">
                        @foreach($ciclosExistentes as $ciclo)
                            <option value="{{ $ciclo }}" {{ $ciclo === $selectedCiclo ? 'selected' : '' }}>{{ $ciclo }}
                            </option>
                        @endforeach
                    </select>
                </div>
            ...
            <a href="{{ route('calificaciones.por_carrera', $carrera) }}" class="btn btn-secondary mt-3">Restablecer</a>
        </form>
        ...
        <table class="table table-bordered">
            ...
            <tbody>
                @foreach($cuatrimestres as $cuatrimestre => $materias)
                        <tr>
                            <td colspan="{{ $calificaciones->count() + 2 }}" class="text-center font-weight-bold">
                                {{ $cuatrimestre }}</td>
                        </tr>
                                ...
                                <tr>
                                    ...
                                    @foreach($calificaciones as $matricula => $materiasAlumno)
                                                ...
                                                <td class="calificacion"
                                                    style="background-color: {{ $isHighlighted ? 'blue' : ($promedioP !== null && $promedioP < 6 ? 'red' : ($isRecentCiclo ? 'lightblue' : 'transparent')) }};">
                                                    @if($promedioP !== null)
                        ...
                        <tr class="font-weight-bold">
                            ...
                            @foreach($calificaciones as $matricula => $materiasAlumno)
                                <td class="text-center">{{ $promediosCuatrimestresPorAlumno[$matricula][$cuatrimestre] ?? 'N/A' }}
                                </td>
                            @endforeach
                            <td></td>
                        </tr>
